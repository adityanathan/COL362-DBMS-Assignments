...
-- 13--
select least((select -1 where not exists (select * from simple_author_paths where authorid1 = 1 and authorid2 = 12)),
(select count(*) from
(
with recursive path(authorid1, authorid2, author_path) as (
    select authorid1, authorid2, array[authorid1, authorid2] -- path grows to the right 
    ...
    where path.authorid2 = ae.authorid1 -- recursion link
        ...
        and ad1.age > 35
        and ((ad2.authorid = 12) or (ad2.age > 35 and ad1.gender <> ad2.gender))
)
select *
from path
where authorid1 = 1 and authorid2 = 12
) temp)) count
;
    ...
    and exists(
        ...
        (
            ...
            where ap.paperid = pp.paperid1
                and pp.paperid2 = 100 -- gives me all the authors who directly or indirectly cited this paper
        ) temp
...
with recursive path(authorid1, authorid2, author_path) as (
    ...
    where path.authorid2 = ae.authorid1 -- recursion link
        ...
        and ((cpa1.total_citations < cpa2.total_citations) or (ae.authorid2 = 12))
)
select * 
from path
where authorid1 = 1 and authorid2 = 12
...
with recursive path(authorid1, authorid2, author_path, city_list, paper_list, cite_list) as (
        ...
        select distinct authorid1, authorid2, 
            ...
            array(select ap.paperid from authorpaperlist ap where ap.authorid = authorid2) as paper_list, -- don't consider first guy
            array(select cl.paperid2 from authorpaperlist ap, citationlist cl where authorid2 = ap.authorid and ap.paperid = cl.paperid1) as cite_list -- don't consider first guy
        ...
        select path.authorid1, ae.authorid2, 
            ...
            (select array(select distinct a from unnest( -- to remove duplicates
                paper_list || array(select ap.paperid from authorpaperlist ap where ap.authorid = ae.authorid2)
            ) as a)),
            (select array(select distinct a from unnest( -- to remove duplicates
                cite_list || array(select cl.paperid2 from authorpaperlist ap, citationlist cl where ae.authorid2 = ap.authorid and ap.paperid = cl.paperid1)
            ) as a))
        ...
        where path.authorid2 = ae.authorid1 -- recursion link
            ...
            and 
            ((ae.authorid2 = 12) or -- exclude first and last guy
            (
                ...
                and not exists(
                    ...
                    where ap.authorid = ae.authorid2
                        and (ap.paperid = ANY(cite_list)) -- this author's papers haven't been cited by previous authors on the path
                )
                and not exists(
                    select cl.paperid2 from authorpaperlist ap, citationlist cl 
                    where ae.authorid2 = ap.authorid and ap.paperid = cl.paperid1
                        and (cl.paperid2 = ANY(paper_list)) -- this author hasn't cited any previous author's papers
                )
            )
            )
)
